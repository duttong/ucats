...
#define		INTPEAKS		3		// number of peaks that can be integrated
#define		MAXECDPTS		16
#define 	CHROMSUB		MAXECDPTS*30	// 16 hz * 30 seconds

double ichrom[CHROMSUB][INTPEAKS];		// chrom array

double prvCalH[INTPEAKS];				// prev cals height
double prvMR[INTPEAKS];					// prev air MR
int intinc[INTPEAKS];
static int init = 0;
...
double stats( int ch, int p1, int p2, int lstpt ) {
	int i;
	double max, min;
	int maxpt, minpt;
...
double subtractbaseline( int ch ) {
	...
	stats( ch, stepin, lstpt-stepin, lstpt );
	top = res.maxpt;

	// left side of peak
	stats( ch, 0, top, lstpt );
	pt1x = res.minpt;
	pt1y = res.min;

	// right side of peak
	stats( ch, top+1, lstpt, lstpt );
	pt2x = res.minpt;
	pt2y = res.min;

	res.m = ( pt2y - pt1y ) / ( pt2x - pt1x );
...
double integrate ( int toFile, long resp, intStruct peak, long t, short ssv ) {
	...
	if ( t < peak.strt ) {
		...
		if ( toFile ) {
			...
			/* for debugging 
			if (( ch == 1 ) && (tmbuf->tm_hour == 18) && (tmbuf->tm_min == 25)) {
				tfile = fopen( "chrom.txt", "a+" );
