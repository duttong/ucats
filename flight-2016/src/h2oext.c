...
RHstruct vai;
 ...
 typedef float vaifloat; /* edf2ext.awk reading h2odata.edf */
  ...
  void initialize(void) {
	...
	ss_set_column(h2oout, 15, "%12.1f", "H2O_Pelt");
	ss_set_column(h2oout, 16, "%12.1f", "H2O_Pelt_Sp");
	ss_set_column(h2oout, 17, "%12.1f", "vaiT");
	ss_set_column(h2oout, 18, "%12.1f", "vaiRH");
  }
  void terminate(void) {
	ss_close(h2oout);
  }
  ...
  if (!(V10)) {
    {
      ...
      ss_set(h2oout, 15, (H2O_Pelt*1E-1));
      ss_set(h2oout, 16, (home->U11.H2O_Pelt_Sp*1E-1));
      ss_set(h2oout, 17, vaiT);
      ss_set(h2oout, 18, vaiRH);
    }
  }
}
