		...
		if ( rcvPid != -1 ) {
			...
			if ( loop == 1 ) {
				...
				if ( (clock()-start_disable_time[chan]) > RESETDELAY * CLOCKS_PER_SEC) {
					temps[2*index] = GetTemp( chan ) * 10;
					if ( temps[2*index] != UNKNOWN )
						temps[2*index+1] = GetSetPoint( chan, 1 ) * 10;
					else
