...
tst = fopen("smart.tst", "w");
	...
	while ( 1 ) {
		...
		if ( rcvPid != -1 ) {
			...
			// Poll electrometers once a second
			if ( detINC >= MAXDET ) detINC = 0;
			if ( ( ValidElec[detINC] ) && 
				 ( (clock() - elecclock[detINC]) > CLOCKS_PER_SEC ) ) {
					elecclock[detINC] = clock();
					read_electrometer( detINC );
#ifdef DEBUG
					printf("det = %d, chrom.numpnts = %d\n", detINC, chrom[detINC].numpnts);
					printf("Detector %d response ( %d data points follow )\n  ", detINC, chrom[detINC].numpnts );
					for(i=0; i<chrom[detINC].numpnts-1; i++)
						printf("%ld,", chrom[detINC].resp[i]);
					printf("%ld\n\n", chrom[detINC].resp[i++]);
#endif
			} else {
				delay(10);
