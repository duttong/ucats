...
unsigned short tti_read_atod( unsigned short channel ) {
  ...
  outpDelay( TT_CFG_PORT, TT_READ_CFG ); /* A in, Ch in, Cl out, B out */
  outpDelay( TT_CTRL_PORT, 1 );   /* STRB/ High */
  outpDelay( TT_ADDR_PORT, channel ); /* output the address */
  outpDelay( TT_CTRL_PORT, 0 );   /* STRB/ Low to start acquisition & conversion */
  start_timer();
  ...
  if ( timed_out ) {
	...
	outpDelay( TT_CTRL_PORT, 2 );   /* Set up for MSNybble */
