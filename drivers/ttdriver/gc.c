...
unsigned short tti_gc_reset( void ) {
  outpDelay( TT_CFG_PORT, TT_READ_CFG );
  outpDelay( TT_CTRL_PORT, 9 );
  outpDelay( TT_ADDR_PORT, TT_GC_BASE );
  outpDelay( TT_CTRL_PORT, 1 );
  delay( 10 ); /* wait for reset delay */
  outpDelay( TT_CTRL_PORT, 9 );
  delay( 100 ); /* wait for hold delay */
  outpDelay( TT_CTRL_PORT, 8 );
  /* look for ACK (CTRL & 0x10)==0 */
  start_timer();
...
unsigned short tti_gc_read( gc_data_buf *gcbuf ) {
  ...
  for ( i = 0; i < N_GC_CHANNELS; i++) {
	...
	{
	  nl_error( 1, "bytes to read was %u", bytes );
	gcbuf->end_offset[0] = ~0;
  outpDelay( TT_ADDR_PORT, TT_DUMMY_ADDR );
  return 0;
