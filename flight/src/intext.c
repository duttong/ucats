    ...
    case 1: /* NotInjecting */ {
        {
        	...
        	if (intrest == 0) {
        		resetintfile( N2Oint ); 
        		resetintfile( CFC12int );
        		resetintfile( SF6int );
        		resetintfile( CH4int );
        		resetintfile( CFC11int );
        		intrest = 1;
        	}
        } { secondhalf = 0; }
      }
  ...
  {if (!(V12)) {
    {
    		...
    		if ( home->U0.GC_Ctr_CH3 != ECDOFFLINE ) {
    			integrate( 1, home->U0.GC_Ctr_CH3, N2Oint, CHROMtime, old_ssv1min );
    		}
    
    		if ( home->U0.GC_Ctr_CH3 != ECDOFFLINE ) {
    			integrate( 1, home->U0.GC_Ctr_CH3, SF6int, CHROMtime, old_ssv1min );
    		}
    
    		if ( home->U0.GC_Ctr_CH2 != ECDOFFLINE ) {
    			integrate( 1, home->U0.GC_Ctr_CH2, CFC12int, CHROMtime, old_ssv1min );
    		}
    
    		if ( home->U0.GC_Ctr_CH2 != ECDOFFLINE ) {
    			integrate( 1, home->U0.GC_Ctr_CH2, CFC11int, CHROMtime, old_ssv1fold );
    		}
