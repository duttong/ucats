...
static int rule_action(unsigned short rule) {
  switch (rule) {
    ...
    case 1:
	  /* &start : &commands Quit * */
	    ...
	    { 
		  ...
		  switch_gsv( CH3gsvADD, 1 ); delay(100);
    ...
    case 16:
	  ...
	  #ifdef SERVER_ACTIONS
	    if (ioflags & IOF_EXECUTE)
	    { ADRdigSet( 1, 2, V(1,tsp).vu03 ); }
	  #else
	    saw_server_action = 1;
	  #endif
	  nterm_shift(4, 16, PP(2,P(1,tsp)));
	  return(0);
    case 17:
	  /* &command : Pump Ozone &On */
	  #ifdef SERVER_ACTIONS
	    if (ioflags & IOF_EXECUTE)
	    { ADRdigSet( 3, 7, V(1,tsp).vu03 ); }
	  #else
	    saw_server_action = 1;
	  #endif
	  nterm_shift(4, 17, PP(2,P(1,tsp)));
	  return(0);
    case 18:
	  /* &command : Pump H2O &On */
	  #ifdef SERVER_ACTIONS
	    if (ioflags & IOF_EXECUTE)
	    { ADRdigSet( 3, 6, V(1,tsp).vu03 ); }
	  #else
	    saw_server_action = 1;
	  #endif
	  nterm_shift(4, 18, PP(2,P(1,tsp)));
	  return(0);
    case 19:
	  /* &command : Valve H2O &Open */
	  #ifdef SERVER_ACTIONS
	    if (ioflags & IOF_EXECUTE)
	    { ADRdigSet( 3, 5, V(1,tsp).vu03 ); }
	  #else
	    saw_server_action = 1;
	  #endif
	  nterm_shift(4, 19, PP(2,P(1,tsp)));
	  return(0);
    case 20:
	  /* &command : Pump All &On */
	  #ifdef SERVER_ACTIONS
	    if (ioflags & IOF_EXECUTE)
	    { ADRdigSet( 1, 2, V(1,tsp).vu03 ); ADRdigSet( 3, 7, V(1,tsp).vu03 ); ADRdigSet( 3, 6, V(1,tsp).vu03 ); ADRdigSet( 3, 5, V(1,tsp).vu03 ); }
	  #else
	    saw_server_action = 1;
	  #endif
	  nterm_shift(4, 20, PP(2,P(1,tsp)));
	  return(0);
    case 21:
	  /* &command : Nitrogen &Open */
	  #ifdef SERVER_ACTIONS
	    if (ioflags & IOF_EXECUTE)
	    { ADRdigSet( 0, 5, V(1,tsp).vu03 ); }
	  #else
	    saw_server_action = 1;
	  #endif
	  nterm_shift(4, 21, PP(1,P(1,tsp)));
	  return(0);
    case 22:
	  /* &command : sample air */
	  #ifdef SERVER_ACTIONS
    ...
    case 23:
	  /* &command : sample cal */
	    ...
	    saw_server_action = 1;
	  #endif
	  nterm_shift(4, 23, PP(3,tsp));
	  return(0);
    case 24:
	  /* &command : solenoid samp In &Open */
	  #ifdef SERVER_ACTIONS
	    if (ioflags & IOF_EXECUTE)
	    { ADRdigSet( 1, 6, V(1,tsp).vu03 ); }
	  #else
	    saw_server_action = 1;
	  #endif
	  nterm_shift(4, 24, PP(3,P(1,tsp)));
	  return(0);
    case 25:
	  /* &command : solenoid samp Out &Open2 */
	  #ifdef SERVER_ACTIONS
	    if (ioflags & IOF_EXECUTE)
	    { ADRdigSet( 1, 7, V(1,tsp).vu03 ); }
	  #else
	    saw_server_action = 1;
	  #endif
	  nterm_shift(4, 25, PP(3,P(1,tsp)));
	  return(0);
    case 26:
	  /* &command : solenoid samp both Open */
	  #ifdef SERVER_ACTIONS
	    if (ioflags & IOF_EXECUTE)
	    {	ADRdigSet( 1, 6, 1 ); ADRdigSet( 1, 7, 0 ); }
	  #else
	    saw_server_action = 1;
	  #endif
	  nterm_shift(4, 26, PP(5,tsp));
	  return(0);
    case 27:
	  /* &command : solenoid samp both Close */
	  #ifdef SERVER_ACTIONS
	    if (ioflags & IOF_EXECUTE)
	    { ADRdigSet( 1, 6, 0 ); ADRdigSet( 1, 7, 1 ); }
	  #else
	    saw_server_action = 1;
	  #endif
	  nterm_shift(4, 27, PP(5,tsp));
	  return(0);
    case 28:
    ...
    case 31:
	  /* &command : elec bias &elecnum &bias_change * */
	    ...
	    saw_server_action = 1;
	  #endif
	  nterm_shift(4, 31, PP(2,P(0,P(2,tsp))));
	  return(0);
    case 32:
	  /* &command : elec bias NULL * */
	    ...
	    saw_server_action = 1;
	  #endif
	  nterm_shift(4, 32, PP(5,tsp));
	  return(0);
    case 33:
	  /* &command : flow main channel 1 &FlowSetpoint */
	  #ifdef SERVER_ACTIONS
	    if (ioflags & IOF_EXECUTE)
	    { SetFlowPN( 1, 0, V(1,tsp).vu02 ); }
	  #else
	    saw_server_action = 1;
	  #endif
    ...
    case 34:
	  ...
	  return(0);
    case 35:
	  /* &command : flow main channel 3 &FlowSetpoint */
	  #ifdef SERVER_ACTIONS
	    if (ioflags & IOF_EXECUTE)
	    { SetFlowPN( 3, 0, V(1,tsp).vu02 ); }
	  #else
	    saw_server_action = 1;
	  #endif
	  nterm_shift(4, 35, PP(4,P(1,tsp)));
	  return(0);
    case 36:
	  /* &command : flow backflush channel 1 &FlowSetpoint */
	  #ifdef SERVER_ACTIONS
	    if (ioflags & IOF_EXECUTE)
	    { SetFlowPN( 1, 1, V(1,tsp).vu02 ); }
	  #else
	    saw_server_action = 1;
	  #endif
	  nterm_shift(4, 36, PP(4,P(1,tsp)));
	  return(0);
    case 37:
	  /* &command : flow backflush channel 2 &FlowSetpoint */
	  #ifdef SERVER_ACTIONS
	    if (ioflags & IOF_EXECUTE)
	    { SetFlowPN( 2, 1, V(1,tsp).vu02 ); }
	  #else
	    saw_server_action = 1;
	  #endif
	  nterm_shift(4, 37, PP(4,P(1,tsp)));
	  return(0);
    case 38:
	  /* &command : flow backflush channel 3 &FlowSetpoint */
	  #ifdef SERVER_ACTIONS
	    if (ioflags & IOF_EXECUTE)
	    { SetFlowPN( 3, 1, V(1,tsp).vu02 ); }
	  #else
	    saw_server_action = 1;
	  #endif
	  nterm_shift(4, 38, PP(4,P(1,tsp)));
	  return(0);
    case 39:
	  /* &command : press backflush channel 1 setpoint &PressSetpointBF */
	  #ifdef SERVER_ACTIONS
	    if (ioflags & IOF_EXECUTE)
	    { SetPressBF( 0, 0, V(1,tsp).vu02 ); }
	  #else
	    saw_server_action = 1;
	  #endif
	  nterm_shift(4, 39, PP(5,P(1,tsp)));
	  return(0);
    case 40:
	  /* &command : press backflush channel 2 setpoint &PressSetpointBF */
	  #ifdef SERVER_ACTIONS
	    if (ioflags & IOF_EXECUTE)
	    { SetPressBF( 0, 1, V(1,tsp).vu02 ); }
	  #else
	    saw_server_action = 1;
	  #endif
	  nterm_shift(4, 40, PP(5,P(1,tsp)));
	  return(0);
    case 41:
    ...
    case 44:
	  /* &command : adr2000 &ADRBOARD set DIG &ADRCHAN to &On * */
	    ...
	    saw_server_action = 1;
	  #endif
	  nterm_shift(4, 44, PP(1,P(2,P(1,P(2,tsp)))));
	  return(0);
    case 45:
	  /* &command : adr2000 &ADRBOARD set DtoA &ADRDACCHAN to &ADRvolts * */
	    ...
	    saw_server_action = 1;
	  #endif
	  nterm_shift(4, 45, PP(1,P(2,P(1,P(2,tsp)))));
	  return(0);
    case 46:
    ...
    case 56:
	  ...
	  #endif
	  vsp++;
	  nterm_shift(12, 56, PP(2,tsp));
	  return(0);
    case 57:
	  /* &elecnum : 3 */
	  #ifdef SERVER_ACTIONS
	    if (ioflags & IOF_EXECUTE)
	    { V(0,tsp).vu03 = (CH3elecADD); }
	  #else
	    saw_server_action = 1;
	  #endif
	  vsp++;
	  nterm_shift(12, 57, PP(2,tsp));
	  return(0);
    case 58:
	  /* &ADRBOARD : board 0 */
	  #ifdef SERVER_ACTIONS
    ...
    case 72:
	  ...
	  #endif
	  vsp++;
	  nterm_shift(20, 72, PP(2,tsp));
	  return(0);
    case 73:
	  /* &OMEGAaddress : CH3 ECD */
	  #ifdef SERVER_ACTIONS
	    if (ioflags & IOF_EXECUTE)
	    { V(0,tsp).vu03 = 1; }
	  #else
	  ...
	  #endif
	  vsp++;
	  nterm_shift(16, 73, PP(3,tsp));
	  return(0);
    case 74:
	  /* &OMEGAaddress : CH3 Col */
	  #ifdef SERVER_ACTIONS
	    if (ioflags & IOF_EXECUTE)
	    { V(0,tsp).vu03 = 2; }
	  #else
	  ...
	  #endif
	  vsp++;
	  nterm_shift(16, 74, PP(3,tsp));
	  return(0);
    case 75:
	  /* &OMEGAaddress : CH3 Post */
	  #ifdef SERVER_ACTIONS
	    if (ioflags & IOF_EXECUTE)
	    { V(0,tsp).vu03 = 3; }
	  #else
    ...
    case 91:
	  ...
	  #endif
	  vsp++;
	  nterm_shift(23, 91, PP(2,tsp));
	  return(0);
    case 92:
	  /* &PressSetpointBF : %f (0 to 100 psi) */
	  #ifdef SERVER_ACTIONS
	    if (ioflags & IOF_EXECUTE)
	    { 
		  if (V(1,tsp).vu02 < 0) 
			  V(0,tsp).vu02 = 0.0;
		  else if (V(1,tsp).vu02 > 100) 
			  V(0,tsp).vu02 = 100;
		  else
			  V(0,tsp).vu02 = V(1,tsp).vu02;
	    }
    ...
    case 93:
	  ...
	  #endif
	  vsp++;
	  nterm_shift(11, 93, PP(2,P(1,tsp)));
	  return(0);
    case 94:
	  ...
	  #endif
	  vsp++;
	  nterm_shift(11, 94, PP(2,P(1,tsp)));
	  return(0);
    case 95:
	  /* &GSVADDRESS : channel 3 &GSVVALVEPOS_Inv */
	  #ifdef SERVER_ACTIONS
	    if (ioflags & IOF_EXECUTE)
	    { switch_gsv( CH3gsvADD, V(1,tsp).vu03 ); }
	  #else
	    saw_server_action = 1;
	  #endif
	  vsp++;
	  nterm_shift(11, 95, PP(2,P(1,tsp)));
	  return(0);
    case 96:
	  /* &GSVVALVEPOS : Inject */
	  #ifdef SERVER_ACTIONS
