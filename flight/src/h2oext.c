...
RHstruct vai;
 ...
 typedef float vaifloat; /* edf2ext.awk reading h2odata.edf */
  #include <stdlib.h>
  #include "ssp.h"
  #include "tmctime.h"
...
static void BF8_1(void) {
  tdlrec = home->U23.tdlrec;
  tdl_time = home->U21.tdl_time;
  tdl_shortH2O = home->U17.tdl_shortH2O;
  tdl_H2OB = home->U19.tdl_H2OB;
  ...
  if (!(V10)) {
    {
      ...
      ss_set(h2oout, 1, tdlrec);
      ss_set(h2oout, 2, tdl_time);
      ss_set(h2oout, 3, tdl_shortH2O);
      ss_set(h2oout, 4, tdl_H2OB);
      ss_set(h2oout, 5, tdl_longH2O);
      ss_set(h2oout, 6, tdl_H2OCD);
      ss_set(h2oout, 7, tdl_p);
      ss_set(h2oout, 8, tdl_t);
      ss_set(h2oout, 9, tdl_elecT);
      ss_set(h2oout, 10, tdl_amp);
      ss_set(h2oout, 11, tdl_pow);
      ss_set(h2oout, 12, tdl_pos);
      ss_set(h2oout, 13, tdl_zer);
      ss_set(h2oout, 14, tdl_posB);
      ss_set(h2oout, 15, home->U25.vaiT);
      ss_set(h2oout, 16, home->U25.vaiRH);
      ss_set(h2oout, 17, tdl_best);
    }
  }
}
