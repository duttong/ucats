...
static void BF16_12(void) {
  EF1_0();
  if (!(V11)) {
    { if ( (_CVT_0(home->U11.presL_N2) < N2fail) && (_CVT_0(home->U11.presL_N2) > 0) ) {
    			msg( -2, "** (%d) LOW N2 Pressure (presL_N2 = %f)", N2lowCount,_CVT_0(home->U11.presL_N2) );
    			if ( ++N2lowCount >= MAXN2LOWCNTS ) VSET_8_(2); 
    		  }
    		  else if (_CVT_0(home->U11.presL_N2) >= N2fail ) 
    			N2lowCount = 0; 
    		}
  }
}
...
static void BF16_14(void) {
  ...
  if (!(V12)) {
    { if ( home->U13.InitShutdown == FALSE ) VSET_9_(2); }
  }
}
